...
class PMFEmbedding:
    ...
    def __init__(self, path, normalize=True):
        ...
        with open(path) as f:
            for eachline in f:
                pmfVocab.append(eachline.split()[0])
                pmfVectors.append([float(str) for str in eachline.split()[1:]])

        self.m = np.array(pmfVectors)
        
        if normalize:
            self.normalize()
        self.dim = self.m.shape[1]
        
        (self.wi, self.iw) = dict([(a, i) for i, a in enumerate(pmfVocab)]), pmfVocab
    ...
    def represent(self, w):#input a word, return its vectors or a list of zero
        if w in self.wi:
...
class SVDEmbedding(Embedding):
    ...
    def __init__(self, path, normalize=True, eig=0.0, transpose=False):
        if transpose:
            ...
            #read file, return .wi: dict, word: index; .iw, a list of words
        else:
...
class ConcatenateEmbedding(Embedding):
    """
    ...
    def __init__(self, emb1, emb2, normalize=False):
        ...
        self.m = np.hstack((emb1.m, emb2.m))
